forlap = 'https://forlap.ristekdikti.go.id/mahasiswa/detail/QTE0QzkyREQtOURBMi00OTRGLUIyM0YtMkQwMkMyNzJFRUE0'
import tabulate
...
def data_forlap(link_forlap):
	...
	if l.status == 200:
		...
		datat1 = []
		T1 = s.select('table')[0]
		for i in range(len(T1.select('tr'))):
			datat1.append([])
			if i != 2:
				for j in range(3):
					datat1[i].append(T1.select('tr')[i].select('td')[j].string)
		...
		for i in range(1, len(T2.select('tr'))): ##Mengulang dari 1 hingga jumlah baris pada table, untuk mengambil isi semua td
			for isi in range(len(T2.select('tr')[1].select('td'))):
				datat2[i].append(T2.select('tr')[i].select('td')[isi].string)
		...
		for i in range(1, len(T3.select('tr'))): ##Mengulang dari 1 hingga jumlah table, untuk mengambil isi semua td
			for isi in range(len(T3.select('tr')[1].select('td'))):
				datat3[i].append(T3.select('tr')[i].select('td')[isi].string)
